...
;(async () => {
  ...
  // injectTurn verify-gate), falling back to plain isActive on an older bridge.
  const focusedOf = list => list.find(t => t.isActive && t.groupActive) || list.find(t => t.isActive)
  const tabs = await ci.listChatTabs()
  console.log('LIVE TABS:')
  tabs.forEach(t => console.log('   idx' + t.index, 'vc' + t.viewColumn, t.isActive ? (t.groupActive ? '[FOCUSED]' : '[grp-bg active]') : '         ', JSON.stringify((t.label || '').slice(0, 34))))
  ...
  if (doSubmit) {
    ...
    const justBefore = focusedOf(await ci.listChatTabs())
